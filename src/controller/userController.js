    ...
    try {
       ...
       //check email is present
        ...
        if(address !== undefined){
            if(address!=="object"){return res.status(400).send({ status:false ,message:"address Object"})};
...
const loginUser = async function (req, res) {
    ...
    let data=req.body;
     ...
     if (!user){ return res.status(401).send({ status: false, message: "credentials are not correct" });};
